...
Item {
    ...
    Rectangle {
        ...
        Row {
            ...
            Repeater {
                model: [
                    { icon: "📹", label: "在线设备", value: deviceController.deviceCount, unit: "台", color: "#00D4AA", trend: "+2", trendUp: true },
                    { icon: "🚨", label: "今日告警", value: alarmController.alarmCount, unit: "起", color: "#FF3D71", trend: "-12%", trendUp: false },
                    { icon: "✅", label: "处置率", value: "95", unit: "%", color: "#3B82F6", trend: "+3%", trendUp: true },
                    { icon: "🛡️", label: "安全评分", value: "85", unit: "/100", color: "#FFB800", trend: "+5", trendUp: true },
                    { icon: "🧠", label: "TPU占用", value: statusController.tpuUtilization.toFixed(0), unit: "%", color: "#00D4AA", trend: "", trendUp: true },
                    { icon: "🤖", label: "活跃算法", value: statusController.activeModels, unit: "个", color: "#6C5CE7", trend: "", trendUp: true }
                ]
                ...
                delegate: Rectangle {
                    ...
                    Column {
                        ...
                        Row { spacing: 2; anchors.horizontalCenter: parent.horizontalCenter
                            ...
                            Text { text: modelData.unit; font.pixelSize: 11; color: "#8B8FA3"; anchors.baseline: parent.children[0].baseline }
                        }
                        Row { spacing: 4; visible: modelData.trend !== ""
                            anchors.horizontalCenter: parent.horizontalCenter
                            Text { text: modelData.trendUp ? "↑" : "↓"; font.pixelSize: 10; color: modelData.trendUp ? "#00D4AA" : "#FF3D71" }
                            Text { text: modelData.trend; font.pixelSize: 10; color: modelData.trendUp ? "#00D4AA" : "#FF3D71" }
                        }
                    }
                }
            }
        ...
        ColumnLayout {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    Row { spacing: 4
                        Button { text: "📊 告警统计"; font.pixelSize: 10
                            background: Rectangle { color: "#252830"; radius: 12; width: 68; height: 26; border.color: "#4A4D58"; border.width: 1 }
                        ...
                        Button { text: "📹 设备状态"; font.pixelSize: 10
                            background: Rectangle { color: "#252830"; radius: 12; width: 68; height: 26; border.color: "#4A4D58"; border.width: 1 }
                            contentItem: Text { text: parent.text; font.pixelSize: 10; color: "#8B8FA3"; horizontalAlignment: Text.AlignHCenter; verticalAlignment: Text.AlignVCenter }
            ...
            ScrollView {
                ...
                Column {
                    ...
                    Column { spacing: 4; width: parent.width
                        Repeater {
                            model: [
                                { name: "感知 Agent", icon: "👁️", status: "active", desc: "16路视频实时分析", color: "#00D4AA" },
                                { name: "研判 Agent", icon: "🔍", status: "active", desc: "告警去重+AI研判", color: "#3B82F6" },
                                { name: "决策 Agent", icon: "⚖️", status: "active", desc: "告警分级+联动", color: "#FFB800" },
                                { name: "执行 Agent", icon: "⚡", status: "idle", desc: "等待指令", color: "#8B8FA3" },
                                { name: "元认知 Agent", icon: "🧠", status: "active", desc: "模型自优化R47", color: "#6C5CE7" }
                            ]
                            ...
                            delegate: Rectangle {
                                ...
                                Row {
                                    ...
                                    Text { text: modelData.icon; font.pixelSize: 14 }
                                    Text { text: modelData.name; font.pixelSize: 11; color: "#E8E8E8"; font.bold: true; width: 80 }
                                    ...
                                    Text { text: modelData.status === "active" ? "运行中" : "空闲"; font.pixelSize: 10; color: modelData.color }
                                    Text { text: modelData.desc; font.pixelSize: 9; color: "#4A4D58"; width: 100; elide: Text.ElideRight }
                                }
                            }
                        }
                    }
    ...
    Rectangle {
        ...
        RowLayout {
            ...
            // 30秒自动刷新
            Text { id: refreshCountdown; text: "🔄 30s"; font.pixelSize: 10; color: "#4A4D58" }
        }
    }
